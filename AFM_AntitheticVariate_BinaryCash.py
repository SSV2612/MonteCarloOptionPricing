...
# Calculation of exact option price for binary cash-or-nothing call option
...
# Antithetic variance reduction technique for binary cash-or-nothing
def antithetic_binary_cash_or_nothing(S, K, T, r, sigma, num_paths, num_steps, cash_amount=1):
    """
    Inputs:
    S: Current stock price
    K: Strike price
    ...
    binary_cash_prices_negated_path = np.zeros(num_paths)
    final_option_prices = np.zeros(num_paths)

    for i in range(num_paths):
        X_path = np.zeros(num_steps + 1)
        X_negatedPath = np.zeros(num_steps+1)
        X_path[0] = S
        X_negatedPath[0] = S
        ...
        for j in range(1, num_steps + 1):
            ...
            X_negatedPath[j] = X_negatedPath[j - 1] * np.exp((r-0.5*sigma**2) * dt - sigma * np.sqrt(dt) * standard_normal_array)
        ...
        # Check if the option is in the money at maturity (T) for the path with opposite random numbers used in true path -- Antithetic path option price
        if X_negatedPath[-1] >= K:
            binary_cash_prices_negated_path[i] = cash_amount * np.exp(-r * T)
        else:
            binary_cash_prices_negated_path[i] = 0

    final_option_prices = (binary_cash_prices + binary_cash_prices_negated_path)/2

    # Calculate the average of all binary cash-or-nothing option prices
    average_binary_cash_price = np.mean(final_option_prices)

    return average_binary_cash_price, final_option_prices
...
# Run simulations
monte_binary_cash,binary_cash = antithetic_binary_cash_or_nothing(S, K, T, r, sigma, num_paths, num_steps)
...
print("Absolute Error (Binary Cash-or-Nothing):", absolute_error_binary_cash)
print("Relative Error (Binary Cash-or-Nothing):", relative_error_binary_cash)


for num_paths in num_paths_list:
    # binary cash-or-nothing option price calculation using antithetic variates
    average_monte_price, monte_prices = antithetic_binary_cash_or_nothing(S, K, T, r, sigma, num_paths, num_steps)
...
plt.title("Monte Carlo Option Price vs. Number of Paths with 95% Confidence Intervals")
