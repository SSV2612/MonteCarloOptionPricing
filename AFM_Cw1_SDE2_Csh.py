...
def monte_carlo_binary_cash_SDE2(S, K, T, r, sigma, num_paths, num_steps, cash_amount=1):
    """
    Inputs:
    S: Current stock price
    K: Strike price
...
num_steps_list = [2, 3, 4, 6]
option_prices_list = []
confidence_intervals = []
absolute_error_list = []
delta_list = []

monte, monte_prices = monte_carlo_binary_cash_SDE2(S, K, T, r, sigma, num_paths, num_steps)
...
for num_steps in num_steps_list:
    average_monte_price, monte_prices = monte_carlo_binary_cash_SDE2(S, K, T, r, sigma, num_paths, num_steps)
...
plt.plot(delta_list, absolute_error_list, label="Monte Carlo Option Price", marker='o')
# Set both x-axis and y-axis scales to logarithmic
plt.xscale('log')
plt.yscale('log')

# Draw vertical lines for confidence intervals
for i,num_steps in enumerate(num_steps_list):
    plt.vlines(T/num_steps, confidence_intervals[0, i], confidence_intervals[1, i], colors='red', linestyles='dashed', alpha=0.7, label=f"{0.95*100}% CI")
    print(f"Absolute error value {absolute_error_list[i]}")
...
num_paths_list = [400, 1000, 3000, 4500, 6000, 9000, 10000]
