...
# Antithetic variance reduction technique for plain vanilla call option
def antithetic_monte_carlo_simulation(S, K, T, r, sigma, num_paths, num_steps):
   """
    Inputs:
    S: Current stock price
    K: Strike price
   ...
   antithetic_option_prices = np.zeros(num_paths)
   final_option_prices = np.zeros(num_paths)

   for i in range(num_paths):
      #Construct pair of paths
      S_Path = np.zeros(num_steps+1)
      S_negatedPath = np.zeros(num_steps+1)

      #Set initial stock price at time 0 for each path (true and antithetic path)
      S_Path[0] = S
      S_negatedPath[0] = S
      ...
      for j in range(1,num_steps+1):
         ...
         #Creating an exact opposite path for the same random numbers -- Antithetic path
         S_negatedPath[j] = S_negatedPath[j - 1] * np.exp((r-0.5*sigma**2) * dt - sigma * np.sqrt(dt) * standard_normal_array)
      
      #Option price calculation for antithetic pair at maturity
      option_prices[i] = max(0, S_Path[-1] - K) * np.exp(-r * T)
      antithetic_option_prices[i] = max(0, S_negatedPath[-1] - K) * np.exp(-r * T)

   # Call option price calculation using both paths (true and antithetic)   
   final_option_prices = (option_prices + antithetic_option_prices)/2 

   #Average option price calculation for call option prices using antithetic tech. 
...
monte, monte_prices = antithetic_monte_carlo_simulation(S, K, T, r, sigma, num_paths, num_steps)
...
for num_paths in num_paths_list:
    average_monte_price, monte_prices = antithetic_monte_carlo_simulation(S, K, T, r, sigma, num_paths, num_steps)
